...
describe("Transactional E2E (write)", () => {
  it("sends coins and validates balances + receipt", async (t: { skip: (msg: string) => void }) => {
  ...
  it("sends coins with remarks and verifies remarks on read-back", async (t: { skip: (msg: string) => void }) => {
    logSuite("Transactional E2E (write)");
  ...
  it("deploys a contract (solc) and validates code + storage mutation", async (t: { skip: (msg: string) => void }) => {
