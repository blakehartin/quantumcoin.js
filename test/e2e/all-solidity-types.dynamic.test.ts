 ...
 * @transactional true
 * Dynamic Contract + ContractFactory test for AllSolidityTypes.
 */
...
import { logSuite, logTest, logAddress, logTxn } from "../verbose-logger";
import {
  buildAllUints,
  buildAllInts,
  buildAllFixedBytes,
  ...
  buildOuter,
  canonicalize,
} from "./all-solidity-types.fixtures";
...
function compileAllSolidityTypes({ solcPath, solPath }: { solcPath: string; solPath: string }) {
...
describe("AllSolidityTypes (dynamic Contract)", () => {
  it("deploys and roundtrips all methods", async (t: { skip: (msg: string) => void }) => {
    ...
    (contract as { _deployTx?: unknown })._deployTx = deployTx;

    const unwrap1 = (v: unknown) => (Array.isArray(v) ? v[0] : v);
    ...
    if (Array.isArray(multiC)) {
      ...
      assert.equal((multiC as { outBo: unknown }).outBo === true || (multiC as { outBo: unknown }).outBo === "true", true);
      assert.equal((multiC as { outAddr: string }).outAddr, wallet.address);
      assert.equal((multiC as { outPayableAddr: string }).outPayableAddr, wallet.address);
      assert.equal((multiC as { outB32: unknown }).outB32, canonicalize(expectedFb.b32));
      assert.equal((multiC as { outS: string }).outS, "hello");
      assert.equal((multiC as { outX: string }).outX, "999");
      assert.deepEqual((multiC as { outU: unknown }).outU, canonicalize(expectedU));
    }
