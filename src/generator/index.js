...
function _renderContractDts({ contractName, abi }) {
  ...
  lines.push(`import type { AddressLike } from "./types";`);
  lines.push("");
  ...
  for (const fn of functions) {
    ...
    const argsSig = inputs.map((p, i) => `${_safeIdent(p.name || `arg${i}`)}: ${_solTypeToTs(p.type)}`).join(", ");
...
function _renderFactoryDts({ contractName, abi }) {
  ...
  const ctor = _findConstructor(abi);
  const ctorInputs = (ctor && ctor.inputs) || [];
  const deployArgsSig = ctorInputs.map((p, i) => `${_safeIdent(p.name || `arg${i}`)}: ${_solTypeToTs(p.type)}`).join(", ");
